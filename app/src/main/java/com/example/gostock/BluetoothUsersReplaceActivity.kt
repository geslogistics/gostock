...
class BluetoothUsersReplaceActivity : AppCompatActivity() {
    ...
    private companion object {
        private val MY_UUID: UUID = UUID.fromString("8cc7117b-eca7-4c31-820f-26ed27198bb4") // Unique UUID for user transfer
        private const val APP_NAME = "GoStockUsersReplace"
        private const val TAG = "BluetoothUsersReplace"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_bluetooth_users_replace)
        initUI()
        initBluetooth()
        setupClickListeners()
    ...
    private fun sendData(socket: BluetoothSocket) {
        lifecycleScope.launch(Dispatchers.IO) {
            val userFileHandler = UserFileHandler(this@BluetoothUsersReplaceActivity)
            val usersToSend = userFileHandler.loadUsers()

            if (usersToSend.isEmpty()) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@BluetoothUsersReplaceActivity, "No user data to send.", Toast.LENGTH_SHORT).show()
                }
                return@launch
            }
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@BluetoothUsersReplaceActivity, "✅ User data sent successfully!", Toast.LENGTH_LONG).show()
                    finish()
                }
            } catch (e: IOException) {
    ...
    private fun receiveData(socket: BluetoothSocket) {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val receivedUsers: MutableList<User> = Gson().fromJson(jsonString, listType) ?: mutableListOf()

                if (receivedUsers.isNotEmpty()) {
                    // --- SAFETY CHECK ---
                    // Get the current user on the receiving device
                    val receiverLoggedInUser = GoStockApp.loggedInUser

                    if (receiverLoggedInUser != null) {
                        // Check if the receiver's current user is in the incoming list by ID
                        val userExistsInReceivedData = receivedUsers.any { it.id == receiverLoggedInUser.id }

                        // If the current user is NOT in the new list, add them back to preserve their account.
                        if (!userExistsInReceivedData) {
                            receivedUsers.add(receiverLoggedInUser)
                            Log.i(TAG, "Preserved local user '${receiverLoggedInUser.username}' during user data replacement.")
                        }
                    }
                    // --- END SAFETY CHECK ---

                    val userFileHandler = UserFileHandler(this@BluetoothUsersReplaceActivity)
                    userFileHandler.saveUsers(receivedUsers)

                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@BluetoothUsersReplaceActivity, "✅ User data received and replaced! Please log in again.", Toast.LENGTH_LONG).show()
                        performLogout()
                    }
                } else {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@BluetoothUsersReplaceActivity, "Received empty user list.", Toast.LENGTH_SHORT).show()
                    }
                }
